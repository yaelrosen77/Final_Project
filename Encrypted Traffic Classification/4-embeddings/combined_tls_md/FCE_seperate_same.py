...
class FeatureCombinationEncoder:
    ...
    def combine_features(self, time_series_features, tls_features):
        """Combine processed features into a single embedding."""
        # Process each type of features separately
        processed_time_series = self.process_time_series(time_series_features)
        processed_tls = self.process_tls_features(tls_features)
        # Combine the embeddings
        combined_features = np.concatenate((processed_time_series, processed_tls))
            ...
            for file in files:
                if file.endswith(".xlsx"):
                    ...
                    if len(cat_pro_nav_ope) == 4:
                        ...
                        combined_features = self.combine_features(self.load_time_series(file), self.load_tls_features(file))
                        self.all_embeddings.append(combined_features)
